...
| Bonus | Status | Where |
|---|---|---|
| Event-driven audit trail | ✅ Done | D2 — EligibilityDecidedEvent → WriteAuditEntryHandler |
| Rate limiting on API endpoints | ✅ Done | D20 — Bucket4j filter on /api/** with per-IP + per-actor dimensions |
| Health check + readiness/liveness probes | ✅ Done | application.yml enables endpoints.health/liveness/readiness |
| GitHub Actions CI pipeline | ✅ Done | .github/workflows/build.yml |
| AWS deployment considerations documented (EKS, Secrets Manager) | (deferred to Terraform branch) | feat/terraform-iac |

---

## D20 — Rate limiting: per-IP universal + per-actor on actor endpoints (in-memory Bucket4j, Redis swap deferred)
...
| Redis-backed `RateLimitStore` for multi-instance EKS deploys | `api/ratelimit/redis/RedisRateLimitStore.java` + ElastiCache module in `infra/terraform/main.tf` | Single-instance correctness is in scope (D20); Redis is the swap path. |
