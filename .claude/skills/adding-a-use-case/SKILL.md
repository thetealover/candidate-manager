...
7. **Write the unit test** under `application/src/test/...` with `@ExtendWith(MockitoExtension.class)`. Mock every port. No Micronaut context, no DB.
8. **Run** `./gradlew :application:test :application:spotlessApply`.
